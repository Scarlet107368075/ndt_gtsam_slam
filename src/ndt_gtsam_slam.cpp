...
NDTMap::NDTMap(ros::NodeHandle nh, ros::NodeHandle pnh) : nh_(nh), pnh_(pnh)
{
  ...
  sub_pc_ = nh_.subscribe<sensor_msgs::PointCloud2>("/lslidar_point_cloud", 10000, boost::bind(&NDTMap::pcCB, this, _1));
...
/**
 * @brief ref. loam -> point_cloud to remove motion distortion (based on the assumption of average motion)
 * 
 */
...
/**
 * @brief Extract surround_point_cloud as target map。
...
/**
 * @brief Save key_frame & corresponding pose, update pose_graph
 * 移动距离作为关键帧选取标准
 * 
 */
bool NDTMap::saveKeyframesAndFactor()
{
  // The current_pose is the ndt final_transformation
...
void NDTMap::pcCB(const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  // point_cloud preprocessing 
  pcl::PointCloud<PointT>::Ptr tmp_cloud(new pcl::PointCloud<PointT>());

  pc_source_->clear();
  pcl::fromROSMsg(*msg, *tmp_cloud);
  // coordinate transform (/base_link to /lidar)
  ...
  // First point_cloud frame as target & init pose
  if (cloud_keyframes_.empty())
  {
    ...
    *pc_target_ += *pc_source_;
    ...
    if (use_odom_ && odom_ptr_last_ != -1)
    {
      ...
      odom_ptr_front_ = odom_ptr; // update odom_ptr
    }
  }
  ...
  if (use_odom_ && odom_ptr_last_ != -1)
  {
    ...
    // std::cout << "----------------------------------------" << std::endl;
    // std::cout << "pre_pose_m: " << std::endl;
    // std::cout << pre_pose_m_ << std::endl;
    // std::cout << "cur_pose_m: " << std::endl;
    // std::cout << cur_pose_m_ << std::endl;
    // std::cout << "----------------------------------------" << std::endl;
  ...
  // NDT matching iterate
  cpu_ndt_.setInputSource(pc_source_);
  cpu_ndt_.align(*output_cloud, cur_pose_m_);
  ...
  if (use_odom_ && odom_ptr_last_ != -1)
  {
    ...
    std::cout << "map to odom transform." << std::endl;
  }
  else
  {
    tf_broadcaster_.sendTransform(tf::StampedTransform(tf_m2b, msg->header.stamp, "map", "/odom"));
    std::cout << "map to base_link transform." << std::endl;
  }
  ...
  std::cout << "submap: " << pc_target_->points.size() << " points." << std::endl;
...
/**
 * @brief Loop closure & update pose_graph
 * ICP mathcing add in loop closure
 */
...
/**
 * @brief Loop closure based on odom, straight extract current_pose surround(radiusSearch) keyframe as icp target
 * 
 */
bool NDTMap::detectLoopClosure()
{
  ...
  // Time close don't do loop closure
...
/**
 * @brief After detect loop closure, update pose & target map
 * 
 */
